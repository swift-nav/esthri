...
use esthri::{blocking, sync, SyncParam, INCLUDE_EMPTY, EXCLUDE_EMPTY};
...
fn test_sync_down() {
    ...
    let res = blocking::sync(s3client.as_ref(), source, destination, includes.as_deref(), excludes.as_deref());
...
async fn test_sync_down_async() {
    ...
    let res = sync(s3client.as_ref(), source, destination, includes.as_deref(), excludes.as_deref()).await;
    assert!(res.is_ok(), format!("s3_sync result: {:?}", res));
}
...
fn test_sync_down_fail() {
    ...
    let res = blocking::sync(s3client.as_ref(), source, destination, INCLUDE_EMPTY, EXCLUDE_EMPTY);
    assert!(res.is_err());
}
